...
import { ServerMaintenanceAllowedPlayerQueries } from "@/db/queries/server/maintenance/allowed/player";

/**
 * Namespace queries for server_maintenance_allowed
 *
 ...
 * access to child tables that share the 'server_maintenance_allowed_' prefix.
 *
 ...
 * - Shared across all ServerMaintenanceAllowedQueries instances using same connection
 *
 * Auto-generated from database schema
 * DO NOT EDIT MANUALLY - regenerate with: pnpm generate
 */
export class ServerMaintenanceAllowedQueries {
  /**
   ...
   * - Keys are fully qualified (e.g., "server_maintenance_allowed.actions")
   */
   ...
   *
   * @remarks
   * - Cache key is prefixed with namespace (e.g., "server_maintenance_allowed.actions")
  ...
  protected getOrCreateChild<T>(
    ...
    if (!ServerMaintenanceAllowedQueries.queryInstances.has(this.db)) {
      ServerMaintenanceAllowedQueries.queryInstances.set(this.db, new Map());
    }

    const cache = ServerMaintenanceAllowedQueries.queryInstances.get(this.db)!;
    const fullKey = `server_maintenance_allowed.${key}`;
  ...
  /** Private backing field for lazy-loaded server_maintenance_allowed_player queries */
  private _player?: ServerMaintenanceAllowedPlayerQueries;

  /**
   * Lazy-loaded singleton accessor for server_maintenance_allowed_player
   *
   * Returns a ServerMaintenanceAllowedPlayerQueries instance that shares this namespace's
   * database connection. The instance is created once on first access and
   * cached for all subsequent calls.
   *
   * @returns Singleton ServerMaintenanceAllowedPlayerQueries instance
   */
  get player(): ServerMaintenanceAllowedPlayerQueries {
    if (!this._player) {
      this._player =
        this.getOrCreateChild<ServerMaintenanceAllowedPlayerQueries>(
          "player",
          ServerMaintenanceAllowedPlayerQueries,
        );
    }
    return this._player;
  }
