...
export class ServerAllyQualifiedPlayerQueries extends ServerAllyQualifiedPlayerBaseQueries {
  ...
  /**
   * Returns qualified players joined to the player table for display.
   * isPending=true means the player qualified but is not in any allied party yet.
   */
  async getQualifiedPlayers(serverId: number) {
    const result = await this.db.query<{
      ...
      `SELECT
        qp.player_uuid AS "playerUuid",
        qp.qualified_at AS "qualifiedAt",
        qp.is_pending AS "isPending",
        p.minecraft_username AS "minecraftUsername"
      FROM server_ally_qualified_player qp
      LEFT JOIN player p ON p.minecraft_uuid = qp.player_uuid
      WHERE qp.server_id = $1
