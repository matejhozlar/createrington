...
import { StructurePackQueries } from "@/db/queries/structure/pack";
...
export class StructureQueries {
   ...
   * - Keys are fully qualified (e.g., "structure.actions")
   */
  ...
  protected getOrCreateChild<T>(
    ...
    if (!StructureQueries.queryInstances.has(this.db)) {
      StructureQueries.queryInstances.set(this.db, new Map());
    }

    const cache = StructureQueries.queryInstances.get(this.db)!;
  ...
  /** Private backing field for lazy-loaded structure_pack queries */
  private _pack?: StructurePackQueries;
