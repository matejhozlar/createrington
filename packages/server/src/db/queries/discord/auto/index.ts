...
import { DiscordAutoMessageQueries } from "@/db/queries/discord/auto/message";

/**
 * Namespace queries for discord_auto
 * 
 ...
 * access to child tables that share the 'discord_auto_' prefix.
 * 
 ...
 * - Shared across all DiscordAutoQueries instances using same connection
 * 
 * Auto-generated from database schema
 * DO NOT EDIT MANUALLY - regenerate with: pnpm generate
 */
export class DiscordAutoQueries {
  /**
   ...
   * - Keys are fully qualified (e.g., "discord_auto.actions")
   */
   ...
   * 
   * @remarks
   * - Cache key is prefixed with namespace (e.g., "discord_auto.actions")
  ...
  protected getOrCreateChild<T>(
    ...
    if (!DiscordAutoQueries.queryInstances.has(this.db)) {
      DiscordAutoQueries.queryInstances.set(this.db, new Map());
    }

    const cache = DiscordAutoQueries.queryInstances.get(this.db)!;
    const fullKey = `discord_auto.${key}`;
  ...
  /** Private backing field for lazy-loaded discord_auto_message queries */
  private _message?: DiscordAutoMessageQueries;

  /**
   * Lazy-loaded singleton accessor for discord_auto_message
   * 
   * Returns a DiscordAutoMessageQueries instance that shares this namespace's
   * database connection. The instance is created once on first access and
   * cached for all subsequent calls.
   * 
   * @returns Singleton DiscordAutoMessageQueries instance
   */
  get message(): DiscordAutoMessageQueries {
    if (!this._message) {
      this._message = this.getOrCreateChild<DiscordAutoMessageQueries>('message', DiscordAutoMessageQueries);
    }
