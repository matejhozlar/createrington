...
import { DiscordTopRoleQueries } from "@/db/queries/discord/top/role";

/**
 * Namespace queries for discord_top
 *
 ...
 * access to child tables that share the 'discord_top_' prefix.
 *
 ...
 * - Shared across all DiscordTopQueries instances using same connection
 *
 * Auto-generated from database schema
 * DO NOT EDIT MANUALLY - regenerate with: pnpm generate
 */
export class DiscordTopQueries {
  /**
   ...
   * - Keys are fully qualified (e.g., "discord_top.actions")
   */
   ...
   *
   * @remarks
   * - Cache key is prefixed with namespace (e.g., "discord_top.actions")
  ...
  protected getOrCreateChild<T>(
    ...
    if (!DiscordTopQueries.queryInstances.has(this.db)) {
      DiscordTopQueries.queryInstances.set(this.db, new Map());
    }

    const cache = DiscordTopQueries.queryInstances.get(this.db)!;
    const fullKey = `discord_top.${key}`;
  ...
  /** Private backing field for lazy-loaded discord_top_role queries */
  private _role?: DiscordTopRoleQueries;

  /**
   * Lazy-loaded singleton accessor for discord_top_role
   *
   * Returns a DiscordTopRoleQueries instance that shares this namespace's
   * database connection. The instance is created once on first access and
   * cached for all subsequent calls.
   *
   * @returns Singleton DiscordTopRoleQueries instance
   */
  get role(): DiscordTopRoleQueries {
    if (!this._role) {
      this._role = this.getOrCreateChild<DiscordTopRoleQueries>(
        "role",
        DiscordTopRoleQueries,
      );
    }
    return this._role;
  }
