 ...
 * access to child tables that share the 'discord_guild_member_' prefix.
 *
...
export class DiscordGuildMemberQueries {
   ...
   * - Keys are fully qualified (e.g., "discord_guild_member.actions")
   */
   ...
   *
   * @remarks
   * - Cache key is prefixed with namespace (e.g., "discord_guild_member.actions")
  ...
  /** Private backing field for lazy-loaded discord_guild_member_join queries */
  private _join?: DiscordGuildMemberJoinQueries;

  /**
   * Lazy-loaded singleton accessor for discord_guild_member_join
   *
   * Returns a DiscordGuildMemberJoinQueries instance that shares this namespace's
   * database connection. The instance is created once on first access and
   * cached for all subsequent calls.
   *
   * @returns Singleton DiscordGuildMemberJoinQueries instance
   */
  get join(): DiscordGuildMemberJoinQueries {
    if (!this._join) {
      this._join = this.getOrCreateChild<DiscordGuildMemberJoinQueries>(
        "join",
        DiscordGuildMemberJoinQueries,
      );
    }
    return this._join;
  }

  /** Private backing field for lazy-loaded discord_guild_member_leave queries */
  private _leave?: DiscordGuildMemberLeaveQueries;

  /**
   * Lazy-loaded singleton accessor for discord_guild_member_leave
   *
   * Returns a DiscordGuildMemberLeaveQueries instance that shares this namespace's
   * database connection. The instance is created once on first access and
   * cached for all subsequent calls.
   *
   * @returns Singleton DiscordGuildMemberLeaveQueries instance
   */
  get leave(): DiscordGuildMemberLeaveQueries {
    if (!this._leave) {
      this._leave = this.getOrCreateChild<DiscordGuildMemberLeaveQueries>(
        "leave",
        DiscordGuildMemberLeaveQueries,
      );
    }
    return this._leave;
  }
