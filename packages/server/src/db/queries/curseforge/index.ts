...
/**
 * Namespace queries for curseforge
 *
 ...
 * access to child tables that share the 'curseforge_' prefix.
 *
 ...
 * - Shared across all CurseforgeQueries instances using same connection
 *
 * Auto-generated from database schema
 * DO NOT EDIT MANUALLY - regenerate with: pnpm generate
 */
export class CurseforgeQueries {
  /**
   ...
   * - Keys are fully qualified (e.g., "curseforge.actions")
   */
   ...
   *
   * @remarks
   * - Cache key is prefixed with namespace (e.g., "curseforge.actions")
  ...
  protected getOrCreateChild<T>(
    ...
    if (!CurseforgeQueries.queryInstances.has(this.db)) {
      CurseforgeQueries.queryInstances.set(this.db, new Map());
    }

    const cache = CurseforgeQueries.queryInstances.get(this.db)!;
    const fullKey = `curseforge.${key}`;
  ...
  /**
   * Lazy-loaded singleton accessor for curseforge_project
   *
   * Returns a CurseforgeProjectQueries instance that shares this namespace's
   * database connection. The instance is created once on first access and
   * cached for all subsequent calls.
   *
   * @returns Singleton CurseforgeProjectQueries instance
   */
  get project(): CurseforgeProjectQueries {
    if (!this._project) {
      this._project = this.getOrCreateChild<CurseforgeProjectQueries>(
        "project",
        CurseforgeProjectQueries,
      );
    }
