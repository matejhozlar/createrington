 ...
 * access to child tables that share the 'waitlist_' prefix.
 *
 ...
 * - Shared across all WaitlistQueries instances using same connection
 *
...
export class WaitlistQueries {
   ...
   * - Keys are fully qualified (e.g., "waitlist.actions")
   */
   ...
   *
   * @remarks
   * - Cache key is prefixed with namespace (e.g., "waitlist.actions")
  ...
  /** Private backing field for lazy-loaded waitlist_entry queries */
  private _entry?: WaitlistEntryQueries;

  /**
   * Lazy-loaded singleton accessor for waitlist_entry
   *
   * Returns a WaitlistEntryQueries instance that shares this namespace's
   * database connection. The instance is created once on first access and
   * cached for all subsequent calls.
   *
   * @returns Singleton WaitlistEntryQueries instance
   */
  get entry(): WaitlistEntryQueries {
    if (!this._entry) {
      this._entry = this.getOrCreateChild<WaitlistEntryQueries>(
        "entry",
