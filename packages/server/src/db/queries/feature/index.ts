...
import { FeatureFlagQueries } from "@/db/queries/feature/flag";

/**
 * Namespace queries for feature
 *
 ...
 * access to child tables that share the 'feature_' prefix.
 *
 ...
 * - Shared across all FeatureQueries instances using same connection
 *
...
export class FeatureQueries {
   ...
   * - Keys are fully qualified (e.g., "feature.actions")
   */
   ...
   *
   * @remarks
   * - Cache key is prefixed with namespace (e.g., "feature.actions")
  ...
  protected getOrCreateChild<T>(
    ...
    if (!FeatureQueries.queryInstances.has(this.db)) {
      FeatureQueries.queryInstances.set(this.db, new Map());
    }

    const cache = FeatureQueries.queryInstances.get(this.db)!;
  ...
  /** Private backing field for lazy-loaded feature_flag queries */
  private _flag?: FeatureFlagQueries;

  /**
   * Lazy-loaded singleton accessor for feature_flag
   *
   * Returns a FeatureFlagQueries instance that shares this namespace's
   * database connection. The instance is created once on first access and
   * cached for all subsequent calls.
   *
   * @returns Singleton FeatureFlagQueries instance
   */
  get flag(): FeatureFlagQueries {
    if (!this._flag) {
      this._flag = this.getOrCreateChild<FeatureFlagQueries>(
        "flag",
        FeatureFlagQueries,
      );
    }
