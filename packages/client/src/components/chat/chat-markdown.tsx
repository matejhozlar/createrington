...
type ChatMarkdownVariant = "body" | "embed-title" | "embed-body";
...
function buildComponents(variant: ChatMarkdownVariant): Components {
  ...
  return {
    ...
    a: ({ children, href }) => {
      const isChannelMention = href?.includes("discord.com/channels/") ?? false;
    ...
    code: ({
      ...
      inline ? (
        ...
        <code
          className={cn("block font-mono", isEmbed ? "text-xs" : "text-sm")}
        >
          {children}
        </code>
      ),
    pre: ({ children }) => (
      <pre
        className={cn(
          ...
          isEmbed ? "bg-sidebar p-2 text-xs" : "bg-sidebar-accent text-sm",
        )}
      >
        {children}
      </pre>
    ...
    ul: ({ children }) => (
      <ul
        className={cn(
          ...
          isEmbed ? "text-xs text-muted-foreground" : "text-sm text-foreground",
        )}
      >
        {children}
      </ul>
    ...
    li: ({ children }) => (
      <li className={isEmbed ? "text-muted-foreground" : "text-foreground"}>
        {children}
      </li>
    ),
    blockquote: ({ children }) => (
    ...
    h1: ({ children }) => (
      <h1
        className={cn(
          "my-1.5 font-bold",
          isEmbed ? "text-sm text-muted-foreground" : "text-lg text-foreground",
        )}
      >
        {children}
      </h1>
...
const COMPONENTS: Record<ChatMarkdownVariant, Components> = {
  body: buildComponents("body"),
  "embed-title": buildComponents("embed-title"),
  "embed-body": buildComponents("embed-body"),
};
