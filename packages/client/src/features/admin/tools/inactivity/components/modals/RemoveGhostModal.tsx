...
export function RemoveGhostModal({
  ...
  const removeGhost = trpc.admin.inactivity.ghosts.remove.useMutation({
    ...
    onError: (error) => toast.error(error.message || "Failed to remove ghost"),
  });
  const displayGhost = useStickyValue(ghost);

  const discordId = ghost?.discordId ?? null;

  useEffect(() => {
    if (discordId !== null) {
      verifyGhost.mutate({ discordId });
    } else {
      verifyGhost.reset();
    }
  ...
  return (
    <ConfirmDialog
      ...
      title={<span className="text-destructive">Remove Ghost Player</span>}
      description={
        displayGhost && (
          <>
            This will remove{" "}
            <span className="font-semibold">
              &quot;{displayGhost.minecraftUsername}&quot;
            </span>{" "}
      ...
      onConfirm={() =>
        ghost
          ? removeGhost.mutateAsync({ discordId: ghost.discordId })
          : undefined
      }
    >
      {verifyGhost.isPending ? (
        ...
        <div className="flex items-start gap-3 rounded-lg border border-emerald-500/40 bg-emerald-500/10 p-3 text-sm">
          ...
          <div>
            ...
            <p className="text-emerald-300/80">
              Verification just confirmed this user is not in the guild. Removal
              allowed.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-sm">
          ...
          <div>
            ...
            <p className="text-destructive/80">
              This user is back in the guild and was just removed from the ghost
              cache. Close this dialog and refresh the list.
            </p>
          </div>
        </div>
      ) : null}
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-mono text-xs">
                {displayGhost.discordId}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Last seen:</span>
              <span className="font-medium">
                {new Date(displayGhost.playerLastSeen).toLocaleDateString()}
              </span>
            </div>
          </div>
        </div>
