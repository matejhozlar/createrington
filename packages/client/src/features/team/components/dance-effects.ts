...
export class DiscoAnimation extends PlayerAnimation {
  protected animate(player: PlayerObject): void {
    ...
    if (t < DANCE_END) {
      ...
      const spinW =
        smoothstep(POINT_IN_SEGMENT - FADE, POINT_IN_SEGMENT + FADE, segTime) *
      ...
      // ── Compute spin pose ──
      const spinLocal =
        (segTime - POINT_IN_SEGMENT) / (SEGMENT - POINT_IN_SEGMENT);
      ...
      const sArmRZ = -0.7;
      const sPosY =
        Math.sin(Math.max(0, Math.min(1, spinLocal)) * Math.PI) * 0.4;
