...
export function SuggestionPanel({
  ...
  vote: { id: number; maxModsPerUser: number };
}) {
  ...
  const invalidate = () => {
    utils.user.votes.mySuggestions.invalidate({ voteId: vote.id });
    utils.user.votes.searchProjects.invalidate();
          ...
          <div className="space-y-2">
            {suggestions.map((mod) => {
              ...
              const removing =
                ...
                removeMutation.variables?.voteModId === mod.id;
              return (
                <div
                  key={mod.id}
                ...
                >
                  ...
                  {mod.status === "pending" && (
                    <Button
                      ...
                      onClick={() =>
                        removeMutation.mutate({ voteModId: mod.id })
                      }
                    >
                      {removing ? (
